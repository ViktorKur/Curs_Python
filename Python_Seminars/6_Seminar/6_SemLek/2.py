...
list_n = [int(input('Введите элементы массива через Enter: ')) for i in range (int(input('Введите количество элементов списка n: ')))]
list_k = [i for i in range(1, len(list_n) - 1) if list_n[i - 1] < list_n[i] > list_n[i + 1]]
print(f"(кол-во элементов, номер элем):-", list(enumerate(list_k ,1)))